...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM3) {
        /* I2C Vibration Recovery:
           A 14-byte I2C read takes < 2ms. If 10ms have passed and the I2C
           is STILL not ready, it means the bus hung due to physical vibration
           (noise or loose wire). We must forcefully reset the I2C peripheral! */
        if (hi2c1.State != HAL_I2C_STATE_READY) {
            HAL_I2C_DeInit(&hi2c1);
            HAL_I2C_Init(&hi2c1);
        }

        /* Trigger I2C DMA read of MPU6050 data (14 bytes: Accel, Temp, Gyro) */
